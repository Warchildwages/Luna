...
export function lunaErc8004Manifest(): Record<string, unknown> {
  return {
    ...
    identity: {
      ...
      resolverEndpoint: '/.well-known/did/nostr/{pubkey}.json',
      resolverType: 'HTTP',
      alsoKnownAs: [],
    },
    payment: {
    ...
    capabilities: [
      ...
      {
        name: 'create_event',
        description: 'Create events with pricing tiers, dates, and venue info',
        endpoint: '/api/x402/create-event',
        price: 0.05,
        free: false,
      },
      {
        name: 'manage_gig',
        description: 'Post or fill gig openings for performers, photographers, vendors',
        endpoint: '/api/x402/manage-gig',
        price: 0.02,
        free: false,
      },
      {
        name: 'manage_capacity',
        description: 'Update venue capacity and section allocations',
        endpoint: '/api/x402/manage-capacity',
        price: 0.01,
        free: false,
      },
      {
        name: 'set_pricing',
        description: 'Set or update event ticket pricing tiers',
        endpoint: '/api/x402/set-pricing',
        price: 0.01,
        free: false,
      },
      {
        name: 'venue_analytics',
        description: 'Event performance data — tickets, revenue, attendance',
        endpoint: '/api/x402/venue-analytics',
        price: 0.02,
        free: false,
      },
      {
        name: 'process_payout',
        description: 'Process payouts to creators, artists, and vendors',
        endpoint: '/api/x402/process-payout',
        price: 0.05,
        free: false,
      },
      // Social / Experience Operations
      {
        name: 'lookup_contact',
        description: 'Look up a contact from past event attendance history',
        endpoint: '/api/x402/lookup-contact',
        price: 0,
        free: true,
      },
      {
        name: 'pre_verify',
        description: 'Pre-verify attendees for drink passes, age checks, VIP, or add-ons',
        endpoint: '/api/x402/pre-verify',
        price: 0.01,
        free: false,
      },
      {
        name: 'buy_merch',
        description: 'Purchase event merchandise — shirts, hats, posters, gear',
        endpoint: '/api/x402/buy-merch',
        price: 0.02,
        free: false,
      },
      ...
      {
        name: 'list_for_sale',
        description: 'List tickets on secondary marketplace or auction house',
        endpoint: '/api/x402/list-for-sale',
        price: 0.01,
        free: false,
      },
      {
        name: 'cancel_listing',
        description: 'Cancel an active marketplace or auction listing',
        endpoint: '/api/x402/cancel-listing',
        price: 0,
        free: true,
      },
    ],
...
export async function registerLunaOnErc8004(): Promise<Erc8004Registration> {
  ...
  try {
    ...
    // Register on ERC-8004 Identity Registry via viem
    // Uses Luna's EVM private key to sign the registration transaction
    const { createWalletClient, http } = await import('viem');
    const { baseSepolia } = await import('viem/chains');

    const LUNA_EVM_PRIVATE_KEY_Hex = LUNA_EVM_PRIVATE_KEY.startsWith('0x')
      ? (LUNA_EVM_PRIVATE_KEY as `0x${string}`)
      : (`0x${LUNA_EVM_PRIVATE_KEY}` as `0x${string}`);

    const walletClient = createWalletClient({
      account: LUNA_EVM_PRIVATE_KEY_Hex as `0x${string}`,
      chain: baseSepolia,
      transport: http(),
    });

    const ERC8004_IDENTITY_REGISTRY_ADDRESS = ERC8004_IDENTITY_REGISTRY as `0x${string}`;

    // ERC-8004 Identity Registry ABI (minimal — registerAgent + agentOf)
    const erc8004Abi = [
      {
        type: 'function',
        name: 'registerAgent',
    ...
    const txHash = await walletClient.writeContract({
      ...
      args: [LUNA_EVM_WALLET as `0x${string}`, manifestJson],
    });

    console.log(`[erc8004] Registration tx submitted: ${txHash}`);
